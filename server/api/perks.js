...
async function getPerks(params) {
  try {
    const query = {
...
async function getPerkTemplate(params) {
  try {
    const collection = db.collection("perktemplates");

    const myDoc = await collection.findOne({ template_id: params.template_id });
    return myDoc;
  } catch (err) {
    log({
      level: "error",
      message: "Error occurred while fetching perk template.",
      function: "getPerkTemplate",
      params: params,
      error_code: 500,
      error_stack: err.stack,
    });
...
async function getPerkTemplates(params) {
  try {
    const collection = db.collection("perktemplates");
    let query = { date_deleted: { $eq: null } };
    if (params.perk_enabled) {
      query = {
        ...query,
        perk_enabled: true,
      };
    ...
    log({
      level: "error",
      message: "Error occurred while fetching perk templates.",
      function: "getPerkTemplates",
      params: params,
      error_code: 500,
      error_stack: err.stack,
    });
...
async function upsertPerkTemplate(params, authUser) {
  try {
    const date = new Date();
    const templateID =
      params.template_id || crypto.randomBytes(16).toString("hex");
    ...
    const myDoc = await collection.findOne({ template_id: templateID });
    return myDoc;
  } catch (error) {
    log({
      level: "error",
...
async function upsertPerk(params, authUser) {
  try {
    const date = new Date();
